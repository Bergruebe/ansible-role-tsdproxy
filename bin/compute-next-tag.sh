...
#
# Tags look like `v<TSDProxy version>-<release>`. The version value in
# defaults/main.yml carries no leading `v` (TSDProxy's Docker tags do not have
# one), while the role's own tags do, so one is added here:
#
# - if defaults/main.yml points at a TSDProxy version that has never been
#   released, the release counter restarts at 0 (`v1.4.7-0`)
# - otherwise the counter is incremented (`v1.4.7-1`), but only if something
...
# `tsdproxy_version` is the leaf literal that Renovate rewrites (it carries the
# `# renovate:` annotation). Anchored to the start of the line so that the
# derived `tsdproxy_container_image_tag` / `tsdproxy_container_image`, which
# hold Jinja expressions rather than a version, cannot be picked up instead.
version="$(sed -nE 's|^tsdproxy_version:[[:space:]]*"?([^"[:space:]]+)"?.*$|\1|p' "$defaults_path" | head -n1)"

if [ -z "$version" ]; then
	echo >&2 "Could not determine the TSDProxy version from $defaults_path"
	exit 1
fi

# The version values carry no leading `v` (e.g. `1.4.7`) while the tags do.
# Stripped first anyway, so that this keeps working if that ever changes.
